...
import Service.BookTicketByCinemaService;
import entity.User;
import org.json.simple.JSONObject;

public class BookingTicketByCinemaController extends HandlerByToken {
    @Override
    public String doProcessPost(JSONObject params, User user) {
        JSONObject js = new JSONObject();
        if(user.getRole()==1) {
            BookTicketByCinemaService bookTicketByCinemaService = new BookTicketByCinemaService();
            ...
            String ListRegister = bookTicketByCinemaService.BookTicketByCinema(paramIdtiket, paramstatus, paramIdroom, paramIdrap, paramIdmovie, paramIduser);
            return ListRegister;
        }else {
            js.put("rc", "-1");
